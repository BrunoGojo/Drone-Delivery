...
def main():
    ...
    print("--- Supervisório: Sistema de Auto-Reparo (Respawn Base) ---")
    ...
    while True:
        ...
        if not repair_mode:
            if drone.check_crash():
                ...
                drone.execute_repair() # Drone fica estático na base
                energy_consumed += 100.0 # Penalidade alta
        ...
        if repair_mode:
            ...
            if frame_counter % 60 == 0: # Print a cada ~0.25s
                print(f"Reparando... {3.0 - tempo_passado:.1f}s")
                
            if tempo_passado > 3.0:
                print("[SISTEMA] Sistema Reiniciado. Decolando!")
                drone.finish_repair() # Acorda a física
                repair_mode = False
                # Ao voltar pra base, resetamos o alvo atual para a base também
        ...
        # --- TELEMETRIA ---
        # Coletar histórico mesmo no reparo
        z_history.append(current_pos[2])
        if len(z_history) > 50: z_history.pop(0)
        
        # Se estiver em reparo, velocidade é zero
